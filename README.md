# Budgie Webcam Applet

[![GitHub release (latest by date)](https://img.shields.io/github/v/release/pgroenbaek/budgie-webcam-whitebalance?style=flat&label=Latest%20Version)](https://github.com/pgroenbaek/budgie-webcam-whitebalance/releases)
[![License GNU GPL v3](https://img.shields.io/badge/License-%20%20GNU%20GPL%20v3%20-lightgrey?style=flat&logo=data:image/svg+xml;base64,&logoColor=%23ffffff)](/LICENSE)

This is a Budgie panel applet that allows you to adjust for instance the white balance temperature of a webcam or other video device.


## Installing
Find and download the `.deb` package in the [latest release](https://github.com/pgroenbaek/budgie-webcam-whitebalance/releases).

Install using the command:
```bash
sudo dpkg -i budgie-webcam-whitebalance-applet_1.0.0_amd64.deb
```

## Usage
1. Add the applet to your Budgie panel in the *Budgie Desktop Settings* application.

2. Click the tray icon.

![Applet Disabled](./images/Disabled.png)

3. Enable the applet to control the webcam white balance using the "Enable Control" switch shown above.

### Standard Mode

Standard mode allows you to set the white balance temperature value directly. The temperature can be adjusted using the temperature SpinBox.

![Applet Enabled](./images/Enabled.png)

### Auto Mode

Auto mode uses the webcam's built-in ability to detect and set the temperature automatically. The temperature is updated every 15 minutes to account for changes in lighting conditions throughout the day.

A relative temperature offset can be applied to adjust the value detected by the webcam.

![Applet Auto Mode](./images/Auto.png)


## Building
### Prerequisites

Ensure you have the required dependencies installed:

```bash
sudo apt install budgie-desktop budgie-core budgie-core-dev
sudo apt install libgtk-3-dev libglib2.0-dev gettext v4l-utils 
sudo apt install meson ninja-build valac
```


### Build Debian Package
First build the `.so` library:
```bash
meson setup build
ninja -C build
```

Then build a `.deb` package:
```bash
dpkg-buildpackage -us -uc
```

## License
This Budgie panel applet was created by Peter Grønbæk Andersen and is licensed under [GNU GPL v3](/LICENSE).
